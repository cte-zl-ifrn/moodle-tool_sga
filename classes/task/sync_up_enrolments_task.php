...
class sync_up_enrolments_task extends \core\task\scheduled_task
{
    ...
    {
        return get_string('sync_up_enrolments_task', 'tool_sga');
    }
    ...
    public function execute()
    {
        ...
        require_once($CFG->dirroot . "/admin/tool/sga/api/sync_up_enrolments.php");

        $items = $DB->get_records_sql("SELECT * FROM {sga_enrolment_to_sync} WHERE processed = 0 ORDER BY id ASC");

        foreach ($items as $item) {
            try {
                $service = new \tool_sga\sync_up_enrolments_service();
